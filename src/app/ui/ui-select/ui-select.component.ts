...
export class UiSelectComponent implements OnInit {
  ...
  set selectedValue(newValue) {
    if (_isEqual(newValue, this._selectedValue)) {
      return;
    }
   ...
   */
  getLabel(value) {
    return this.valuesArray ? value : value[this.labelProperty];
  }
  ...
  onIsOpenChange() {
    if (!this.toggleEl) {
      this.toggleEl = this.el.nativeElement.getElementsByClassName(
        "dropdown-toggle"
      )[0];
    }
  ...
  setListEls() {
    if (this.dropdownList) {
      ...
      this.listEls = this.dropdownList.nativeElement.getElementsByClassName(
        "dropdown-item"
      );
  ...
  private setScrollMax() {
    this.scrollMax =
      this.dropdownList.nativeElement.scrollHeight -
      this.dropdownList.nativeElement.clientHeight;
  }
}
