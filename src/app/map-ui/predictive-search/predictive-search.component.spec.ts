...
describe('PredictiveSearchComponent', () => {
  ...
  // const keyEvent = new KeyboardEvent('keyup', {target: {value: 't'}});
  ...
  it('should display results on input', fakeAsync(() => {
    inputEl.nativeElement.value = 't';
    inputEl.triggerEventHandler('keyup', null);
    tick();
    fixture.detectChanges();
    expect(fixture.debugElement.query(By.css('ul')) == null).toBeFalsy();
  }));

  it('should display the option field property', fakeAsync(() => {
    inputEl.nativeElement.value = 't';
    inputEl.triggerEventHandler('keyup', null);
    tick();
    fixture.detectChanges();
    const firstResult = fixture.debugElement.query(By.css('li'));
    expect(firstResult.nativeElement.textContent).toContain(defaultOptions[0].name);
  }));
